...
(records, covered_lines, iteration) after every completed
...
def test_progress_callback_fires_after_each_iteration() -> None:
    ...
    ``state.iteration`` and ``state.records`` reflect the just-
        ...
        snapshots.append(
            {
                ...
                "inputs_count": len(state.records),
    ...
    engine.explore(_three_branch_target, {"x": 0}, progress_callback=on_progress)
